        ...
        switch (parsed.command) {
          ...
          case "HOST":
            if (socket.user && socket.user.userId) {
              ...
              sendAndLog(socket, requestBuilder.single(returnGameUsers(socket.game.gameId, socket.command)));
            }
            break;
          case "JOIN":
            if (socket.user && socket.user.userId) {
              ...
              if (socket.user && socket.user.userId) {
                ...
                if (socket.user.userId === currentGames[socket.game.gameId].host) {
                  ...
                  if (true){
                    ...
                    socket.write(requestBuilder.error("GAME", _.countBy(game.players, function(player) {
                      return player.ready;
            ...
            if (socket.user && socket.user.userId) {
              if (socket.game) {
                ...
                if (parsed.request.beaconId) {
                  var currentBeacon = _.find(game.progress.beacons, function(item){
