...
/**
 * Returns parsed JSON object or an error
 * @param  {string} data Any string, prefferably JSON :)
 * @return {object} Error  Parsed JSON or stringified error.
 */
function parser(data, logger) {
  var response;
  try {
    response = JSON.parse(data);
  } catch (err) {
    logger.log('info', err.toString());
    return null;
  }
  return response;
}
