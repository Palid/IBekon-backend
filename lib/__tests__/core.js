...
function socketSetup(context) {
  runs(function() {
    context.p1.on('data', function(data) {
      context.response = data;
    });
    context.p2.on('data', function(data) {
      context.p2Response = data;
    });
  });
...
describe('Socket', function() {
  ...
  describe('HOST command', function() {
    ...
    it('should return an error if user is not authorized', function() {
      ...
      runs(function() {
        this.splitted = requestUtil.splitter(this.response);
        expect(this.splitted[0]).toEqual(requestUtil.error("JOIN", "You have to authorize by CONNECT first.").trim());
      });
      this.p1.write(requestUtil.build({
        command: "JOIN",
        request: {}
    ...
    it('should return an error if game is not found', function() {
      ...
      socketSetup(this);
      connectUser(this, function() {
        expect(self.splitted[1]).toEqual(requestUtil.error('JOIN', 'Game not found.').trim());
      });
      this.p1.write(requestUtil.build({
        command: "JOIN",
        request: {
