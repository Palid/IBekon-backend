...
function connectAndHostGame(context, callback, player) {
  ...
  connectUser(context, function() {
    expect(context.splitted[player][1]).toEqual('{"command":"HOST","status":"OK","response":{"gameId":"mockedID","host":"myUserID","state":"lobby","settings":{"captureTime":5,"afterCaptureDelay":10,"gameTime":10,"victoryPoints":10000,"pointsPerTick":10,"tickPeriod":10,"maxPlayers":2},"beacons":[],"players":[{"userId":"myUserID","ready":false}]},"date":12345}');
    if (callback) callback();
  }, player);
}
...
describe('Socket', function() {
  ...
  describe('JOIN command', function() {
    ...
    it('should allow p2 to join a hosted game', function() {
      ...
      socketSetup(this, 'p2');
      connectUser(this, function() {
        expect(self.splitted.p2[1]).toEqual('{"response":{},"date":12345,"command":"JOIN","status":"OK"}');
      }, 'p2');
