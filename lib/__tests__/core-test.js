...
describe('Socket', function () {
  ...
  describe('fail-safe', function () {
    ...
    it('should return an error if first command is not CONNECT', function () {
      socketSetup(this);
      runs(function () {
        expect(this.response.p1).toEqual(requestUtil.error("IJustWantToCrash_thisSerVer", "You have to authorize by CONNECT first."));
      });
    ...
    it('should ignore wrong data', function () {
      socketSetup(this);
      runs(function () {
        expect(this.response.p1).toEqual(requestUtil.error(undefined, "You have to authorize by CONNECT first."));
      });
      this.p1.write(requestUtil.build({
        request: "213213"
      }));
