...
function gameLogic(socket, data) {
  ...
  splitted.forEach(function(item) {
    ...
    try {
      ...
      socket.write(requestBuilder.error(undefined, "Couldn't parse JSON."));
      return;
    }
      ...
      switch (parsed.command) {
        case "CONNECT":
          if (parsed.request && parsed.request.userId) {
            ...
            socket.write(requestBuilder.error(socket.command, "No userID provided."));
          }
          break;
        case "HOST":
            ...
            if (socket.game) {
              ...
              if (socket.game.state === "lobby") {
                if (socket.game.settings.maxPlayers > socket.game.players.length) {
                  gameUsersRequest = requestBuilder.single(returnGameUsers(gameId, socket.command));
                  socket.game.players.forEach(function(item) {
                    item.socket.write(gameUsersRequest);
                  });
                } else {
                  socket.write(requestBuilder.error(socket.command, "Maximum players amount exceeded."));
                }
              }
              /**
              ...
              **/
                ...
                socket.write(requestBuilder.error(socket.command, "Game already started or finished"));
              }
            } else {
              socket.write(requestBuilder.error(socket.command, "Game not found."));
            }
          }
          break;
        case "LOBBY_UPDATE":
            ...
            if (socket.game) {
              if (socket.game.state === "lobby") {
                ...
                if (playerIndex !== -1) {
                  ...
                  socket.write(requestBuilder.error(socket.command, "You're not in the game " + gameId));
                }
              } {
                socket.write(requestBuilder.error(socket.command, "Game already started or finished."));
              }
          ...
          if (socket.user && socket.user.userId) {
            if (socket.user.userId === socket.game.host) {
              if (_.every(socket.game.players, 'ready')) {
                ...
                socket.write(requestBuilder.error(socket.command, _.countBy(socket.game.players, function(player) {
                  return player.ready;
                })['false'] + " player(s) aren't ready yet."));
              }
            } else {
              socket.write(requestBuilder.error(socket.command, "You're not the host!"));
            }
          }
          break;
        case "CAPTURE":
        ...
        case "PONG":
          ...
          break;
        default:
          socket.write(requestBuilder.error(parsed.command, "You have provided wrong command."));
          break;
      }
    } else {
